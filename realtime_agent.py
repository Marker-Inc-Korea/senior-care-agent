...
# LiveKit의 실시간 음성 처리와 OpenAI Realtime API를 이용하여
# 신원 확인, 건강 체크, 사람 상담원 연결까지 처리합니다.
# 본 예제에서는 noise cancellation 기능도 추가해봤습니다.
...
class CheckInAgent(BaseAgent):
    ...
    async def transfer_call_to_human(self, ctx: RunContext):
        ...
        if job_ctx is None:
            await ctx.session.generate_reply(
                instructions="Say: I can't transfer the call right now."
            )
            return
        ...
        try:
            ...
            await ctx.session.generate_reply(
                instructions="Say: Transferring you to a human agent now. Please hold."
            )
            ...
            await ctx.session.generate_reply(
                instructions="Say: Sorry, something went wrong transferring the call."
            )
    ...
    async def end_call(self, ctx: RunContext):
        ...
        await ctx.session.generate_reply(
            instructions="End the call by saying have a nice day."
        )
        await hangup_call()
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession[UserData](
        ...
        llm=realtime.RealtimeModel(
            voice="alloy",
            # 두 가지 VAD 방식 중 선택 가능:
            # - server_vad: 음성 파형 기반 Voice Activity Detection
            # - semantic_vad: 의미 기반 Voice Activity Detection (더 자연스러운 turn-taking)
            # 아래는 server_vad 예시 (주석 처리됨)
